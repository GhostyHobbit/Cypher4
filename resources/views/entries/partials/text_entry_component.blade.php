    ...
    <form wire:submit.prevent="saveComponent()">
        <div class="mt-4 mb-4">
            <div class="flex gap-2 p-2 border border-b-0 border-accent rounded-t-md bg-background-dark">
                ...
                <button type="button" tabindex="-1" @click="Alpine.raw(editor).chain().focus().toggleHeading({ level: 3 }).run()" :class="{ 'text-text-default': Alpine.raw(editor)?.isActive('heading', { level: 3 }) }" class="px-2 py-1 text-sm hover:bg-background-light rounded transition">
                    H3
                </button>
                <button type="button" tabindex="-1" @click="Alpine.raw(editor).chain().focus().setParagraph().run()" :class="{ 'text-text-default': Alpine.raw(editor)?.isActive('paragraph') }" class="px-2 py-1 text-sm hover:bg-background-light rounded transition">
                    P
                </button>

                <div class="h-6 w-[1px] bg-accent mx-1 align-self-center"></div>
                ...
                <button type="button" tabindex="-1" @click="Alpine.raw(editor).chain().focus().toggleItalic().run()" :class="{ 'text-text-default': Alpine.raw(editor)?.isActive('italic') }" class="px-2 py-1 text-sm hover:bg-background-light rounded transition">
                    <div class="italic">I</div>
                </button>
                <button type="button" tabindex="-1" @click="Alpine.raw(editor).chain().focus().toggleUnderline().run()" :class="{ 'text-text-default': Alpine.raw(editor)?.isActive('underline') }" class="px-2 py-1 text-sm hover:bg-background-light rounded transition">
                    <div class="underline">U</div>
                </button>
                <button type="button" tabindex="-1" @click="Alpine.raw(editor).chain().focus().toggleStrike().run()" :class="{ 'text-text-default': Alpine.raw(editor)?.isActive('strike') }" class="px-2 py-1 text-sm hover:bg-background-light rounded transition">
                ...
                <button type="button" tabindex="-1" @click="Alpine.raw(editor).chain().focus().toggleBulletList().run()" :class="{ 'text-text-default': Alpine.raw(editor)?.isActive('bulletList') }" class="px-2 py-1 text-sm hover:bg-background-light rounded flex items-center gap-1">
                    <x-icons.list-ul class="h-4 w-4 fill-text-default"/>
                </button>
            ...
            <div x-ref="editorContainer"
                 class="prose max-w-none w-full p-4 border border-accent rounded-b-md bg-background-light min-h-[150px] focus:outline-none focus:border-none focus:ring-0"></div>
